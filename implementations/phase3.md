...
- [x] **T3.1.3** Create DTOs: `CreateGraduateProgramRequest`, `GraduateProgramResponse` — with `@Valid` constraints → SPEC-006
- [x] **T3.1.4** Create `GraduateProgramMapper.java` (MapStruct) — Request/Response ↔ Domain → SPEC-006
- [x] **T3.1.5** Create `GraduateProgramController.java` — endpoints `POST /api/programs`, `GET /api/programs`, `GET /api/programs/{id}`, `PUT /api/programs/{id}` — `@PreAuthorize("hasRole('COORDINATOR')")` → SPEC-006
...
- [x] **T3.2.3** Create DTOs: `SetConfigurationParameterRequest`, `ConfigurationParameterResponse` → SPEC-007
- [x] **T3.2.4** Create `ConfigurationParameterController.java` — nested under programs: `POST /api/programs/{id}/parameters`, `GET /api/programs/{id}/parameters`, `GET /api/programs/{id}/parameters/{key}`, `DELETE /api/programs/{id}/parameters/{key}` → SPEC-007
- [x] **T3.2.5** Create `GlobalExceptionHandler.java` — `@ControllerAdvice` for standard error format (`{ "error": "...", "message": "..." }`) → SPEC-007

---

## Deliverables

- [x] **E3.1** GraduateProgram CRUD returns 200/201/404/409 via REST API — Specs: SPEC-006
- [x] **E3.2** ConfigurationParameter CRUD works scoped by program — Specs: SPEC-007
- [x] **E3.3** Duplicate program name returns 409 — Specs: SPEC-006
- [x] **E3.4** Invalid key format returns 400 with descriptive message — Specs: SPEC-007
- [x] **E3.5** GlobalExceptionHandler produces consistent error format — Specs: SPEC-007

---

## Transition Criteria

- [x] `mvn clean verify` passes with ≥80% coverage
- [x] Controller tests (`@WebMvcTest`) cover happy path and error flows from Gherkin features
- [x] Use case tests (Mockito) verify duplicate name, empty fields, UPPER_SNAKE_CASE validation
- [x] All Gherkin scenarios in `graduate_program_management.feature` mapped to test cases
- [x] All Gherkin scenarios in `configuration_parameter_management.feature` mapped to test cases
...
| # | Decision ID | Summary |
|---|-------------|---------|
| 1 | D-011 | `MethodSecurityConfig` permite todo el tráfico HTTP pero habilita `@PreAuthorize`; autenticación real en Fase 6 |
| 2 | D-012 | JaCoCo excluye `*MapperImpl` (código generado por MapStruct) |
